...
Current inventory: **36 implemented**, **13 partial**, and **19 not implemented
...
| Observability | Persistent non-LMS Bliss-row audit | ✅ Implemented | A private JSON ledger retains current and resolved unmatched identities, reasons, metadata, row IDs, and first/last-seen observations across LMS restarts. INFO logs, Extras, and `blissemall status` expose only its count and location. |
