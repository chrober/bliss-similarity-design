...
| Grow a short seed playlist into a full similar playlist | ✅ Available | Plugin 0.11.1 grows an immutable source set to an exact target, selects only current LMS-local analyzed additions, routes the complete membership, and reports fixed-seed relevance separately from final-route flow. A 2-to-25 Preview and its 25-track saved copy were verified on ARM64; a final rerun exposed all seed/local/unique/repeat proofs in Extras. Optional semantic evidence remains an independent enhancement, not an online requirement. |
...
Current inventory: **39 implemented**, **13 partial**, and **18 not implemented
...
| Playlist workflow | Grow a short seed playlist to an exact target | ✅ Implemented | Plugin 0.11.1 connects the exact `T` editor, immutable-seed request, strict result normalization, Seed/Added review labels, form restoration, and verified-copy path. Preview separately reports best/mean/furthest fixed-seed relevance and complete-route transition sum, worst leg, objective, and arc error. The artifact and plugin fail closed unless exact target, every seed once, local additions, unique membership, and artist/album/track repeat proofs pass. The ARM64 2-to-25 Preview and 25-track copy both passed; optional semantic evidence remains a later enhancement. |
...
automatically**, or **Add exactly N tracks**. Automatic mode owns a per-job
bridge budget and contextual trigger percentile; exact-count owns a validated
count, displays the selected playlist's `S - 1` limit and final-size
calculation, and fails instead of accepting a partial result. Both addition
